...
describe("cli", () => {
  ...
  it("prints hidden app signal locators in text output", async () => {
    ...
    expect(stdout.output).toContain("  schemaFact: sf1 pageCheck.schemaFacts[0] source=json-ld types=SoftwareApplication facts=1 selector=script[type=\"application/ld+json\"]:nth-of-type(1)");
  });
...
npx ax-grep https://example.test --agent</code></pre>
  ...
  it("prints top media and resource refs in text output", async () => {
    ...
    expect(stdout.output).toContain("  resource: rs1 pageCheck.resources[0] feed title=\"Example feed\" rel=alternate type=application/rss+xml selector=link[rel=\"alternate\"] <https://example.test/feed.xml> - feed: Example feed rel=alternate type=application/rss+xml https://example.test/feed.xml");
    ...
    expect(stdout.output).toContain("  media: m1 pageCheck.media[0] open-graph alt=\"Share preview chart\" selector=meta[property=\"og:image\"] <https://example.test/share.png> - Share preview chart - https://example.test/share.png");
  });
  ...
  it("prints executable action-target commands in text output", async () => {
    ...
    expect(stdout.output).toContain("  actionTarget: at1 pageCheck.actionTargets[0] search source=json-ld template=https://example.test/search?q={search_term_string} queryInput=required name=search_term_string selector=script[type=\"application/ld+json\"]:nth-of-type(1) - search: Example Docs");
