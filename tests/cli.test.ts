...
npx ax-grep https://example.test --agent</code></pre>
  ...
  it("prints top media and resource refs in text output", async () => {
    ...
    expect(stdout.output).toContain("  resource: rs1 pageCheck.resources[0] kind=feed title=\"Example feed\" rel=alternate type=application/rss+xml selector=link[rel=\"alternate\"] <https://example.test/feed.xml> - feed: Example feed rel=alternate type=application/rss+xml https://example.test/feed.xml");
    ...
    expect(stdout.output).toContain("  media: m1 pageCheck.media[0] kind=open-graph alt=\"Share preview chart\" selector=meta[property=\"og:image\"] <https://example.test/share.png> - Share preview chart - https://example.test/share.png");
  });
