...
Status: about 94% fit for the goal of making `ax-grep --agent` a useful
...
Overall estimate: 94%. This is still conservative because the final goal is
...
| F4: Browser parity evidence set | Started, gated | A small sequential fixture set comparing static output against browser accessibility output. | Overall moved to 94% after `pnpm compare:browser:fixture` passed with five local fixtures and all gaps classified as covered or implemented. | Do not run broad browser checks; each run needs pre/post `pnpm check:processes`. |
...
| 3 | Keep the browser parity fixture set small and local. | 100% | `pnpm compare:browser:fixture` uses local HTML fixtures, one Puppeteer browser, and no external network or `agent-browser` process. | Pre/post process checks are clean and each fixture result is recorded in the gap ledger. | Enabled overall 94%. |
...
| 5 | Recalculate the headline estimate. | 100% | This file records why the estimate moved from 89% to 90%, 91%, 92%, 93%, then 94%. | The estimate explains why it moved, held, or dropped. | Overall moves only after evidence, not optimism. |
...
| 30 | Later | Add the next local browser fixture only after the current five-fixture gate remains stable. | Pre/post `pnpm check:processes`; one browser-backed command per run. | Can raise or lower estimate. |
