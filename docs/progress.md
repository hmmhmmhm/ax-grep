...
| Semantic accessibility signals | 84% | Landmarks, headings, links, interactive/focusable controls, buttons, fields, values, relations, choices, states, list item refs, first list item shortcuts, table/list container selectors in brief mode, table header/cell navigation shortcuts, table ownership refs, and brief-mode semantic selectors/states are exposed. |
...
| 10 | Later | Compare one semantic table/list/control fixture against browser-tree output, sequentially only. | Pre/post `pnpm check:processes`; smallest comparison command recorded before use. | 1-3% semantic/browser parity. |
| 11 | Later | Recalculate the overall estimate after each landed shortcut or documented defer/browser-only decision. | Update this file in the same commit as the evidence. | Can raise or lower estimate. |
