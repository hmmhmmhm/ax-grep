...
Status: about 93% fit for the goal of making `ax-grep --agent` a useful
...
| Page check handoff | 94% | Forms, action targets, FAQ answers, code block text, resource titles, media/section text, and structured metadata labels/selectors in brief mode, table navigation shortcuts, brief-mode table path/selector targeting, hidden signal group counts/top shortcuts/selectors, author/source metadata, static-readiness reason codes/reasons, fetched-HTML usability in brief mode, barrier-specific browser-capture reason codes, execution shortcuts, barriers, and read targets are exposed in full and brief modes. |
| Semantic accessibility signals | 92% | Landmarks, headings, links, interactive/focusable controls, keyboard shortcut selectors in brief mode, buttons, fields, values, relations, choices, selected-choice shortcuts with controlled targets, states, modal/live state shortcuts, list item refs, first list item shortcuts, list samples from compacted link items, interactive/focusable/actionable selectors in brief mode, table/list names and container selectors in brief mode, table header/cell navigation shortcuts, sample cell spans, table ownership refs, and brief-mode semantic selectors/states are exposed. |
| Browser-tree parity research | 80% | Four local browser fixtures now compare browser-side semantic output with `--agent-brief`; broader real-page and `agent-browser` comparison remains. |

Overall estimate: 93%. This is still conservative because the final goal is
...
| F3: Semantic table/list/control comparison | Started | Whether static table, list, and control shortcuts cover the browser accessibility-tree signals agents actually use. | Semantic accessibility moved to 92% after the fourth local browser fixture proved selected tab target handoff without parsing relation arrays. | Browser-tree comparison may reveal a new P1 signal, which can hold or lower the estimate until tracked. |
| F4: Browser parity evidence set | Started, gated | A small sequential fixture set comparing static output against browser accessibility output. | Overall moved to 93% after `pnpm compare:browser:fixture` passed with four local fixtures and all gaps classified as covered or implemented. | Do not run broad browser checks; each run needs pre/post `pnpm check:processes`. |
...
| 3 | Keep the browser parity fixture set small and local. | 100% | `pnpm compare:browser:fixture` uses local HTML fixtures, one Puppeteer browser, and no external network or `agent-browser` process. | Pre/post process checks are clean and each fixture result is recorded in the gap ledger. | Enabled overall 93%. |
| 4 | Classify browser-tree gaps. | 100% | The fixture set classified ARIA state normalization, compacted list samples, modal/live shallow shortcuts, selected-choice shortcuts, and selected tab panel targets as `implement`, then landed fixes. | Every gap has priority, evidence, decision, validation command, and estimate impact. | Raised browser parity research to 80%. |
| 5 | Recalculate the headline estimate. | 100% | This file records why the estimate moved from 89% to 90%, 91%, 92%, then 93%. | The estimate explains why it moved, held, or dropped. | Overall moves only after evidence, not optimism. |
...
| Semantic accessibility | 92% | Continue adding high-value shortcuts from roles, states, relations, choices, selected choices and their controlled targets, lists, tables, ownership, controls, and brief-mode handoff output. | Compare more table/grid/list/control output against browser-tree expectations and add only useful static equivalents. | Each accepted signal has a public type, CLI output, compact/brief output, and fixture/test coverage. |
| Browser parity research | 80% | Four local browser fixture cases are in place and passed. | Expand the gap list as research finds new browser accessibility-tree signals; lower estimates if new important gaps appear. | Each gap is tagged `implement`, `browser-only`, or `defer` with priority and evidence. |
...
| P6: General accessibility parity | 80% | Run only small sequential comparisons and record every new signal before implementation. | Add more fixture rows only after the current one-command gate remains clean. | Any browser accessibility-tree signal repeatedly helps agent routing. |
...
| A31 | Tablist selected panel browser parity fixture | 100% | Added a fourth local browser fixture covering selected tabs, `aria-controls`, and resolved `tabpanel` targets against `--agent-brief`. | Add another fixture only after the current four-fixture gate remains stable and process checks stay clean. | `pnpm compare:browser:fixture` passes with four local fixtures and clean pre/post process checks. | +5% browser parity research. |
...
| 28 | Later | Add the next local browser fixture only after the current four-fixture gate remains stable. | Pre/post `pnpm check:processes`; one browser-backed command per run. | Can raise or lower estimate. |
