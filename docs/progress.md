...
Status: about 90% fit for the goal of making `ax-grep --agent` a useful
...
| Semantic accessibility signals | 89% | Landmarks, headings, links, interactive/focusable controls, keyboard shortcut selectors in brief mode, buttons, fields, values, relations, choices, states, list item refs, first list item shortcuts, list samples from compacted link items, interactive/focusable/actionable selectors in brief mode, table/list names and container selectors in brief mode, table header/cell navigation shortcuts, sample cell spans, table ownership refs, and brief-mode semantic selectors/states are exposed. |
| Browser-tree parity research | 65% | A single local browser fixture gate now compares browser-side semantic output with `--agent-brief`; broader real-page and `agent-browser` comparison remains. |

Overall estimate: 90%. This is still conservative because the final goal is
...
| F3: Semantic table/list/control comparison | Started | Whether static table, list, and control shortcuts cover the browser accessibility-tree signals agents actually use. | Semantic accessibility moved to 89% after the first local browser fixture gate and list-sample fallback fix. | Browser-tree comparison may reveal a new P1 signal, which can hold or lower the estimate until tracked. |
| F4: Browser parity evidence set | Started, gated | A small sequential fixture set comparing static output against browser accessibility output. | Overall moved to 90% after `pnpm compare:browser:fixture` passed with all gaps classified as covered or implemented. | Do not run broad browser checks; each run needs pre/post `pnpm check:processes`. |
...
| 3 | Pick the smallest browser parity fixture. | 100% | `pnpm compare:browser:fixture` uses one local HTML fixture, one Puppeteer browser, and no external network or `agent-browser` process. | Pre/post process checks are clean and the fixture result is recorded in the gap ledger. | Enabled overall 90%. |
| 4 | Classify first browser-tree gaps. | 100% | The first fixture classified ARIA state normalization and compacted list sample loss as `implement`, then landed fixes. | Every gap has priority, evidence, decision, validation command, and estimate impact. | Raised browser parity research to 65%. |
| 5 | Recalculate the headline estimate. | 90% | This file records why the estimate moved from 89% to 90%. | The estimate explains why it moved, held, or dropped. | Overall moves only after evidence, not optimism. |
...
| Semantic accessibility | 89% | Continue adding high-value shortcuts from roles, states, relations, lists, tables, ownership, controls, and brief-mode handoff output. | Compare more table/grid/list/control output against browser-tree expectations and add only useful static equivalents. | Each accepted signal has a public type, CLI output, compact/brief output, and fixture/test coverage. |
| Browser parity research | 65% | First local browser fixture gate is in place and passed. | Expand the gap list as research finds new browser accessibility-tree signals; lower estimates if new important gaps appear. | Each gap is tagged `implement`, `browser-only`, or `defer` with priority and evidence. |
...
| P6: General accessibility parity | 65% | Run only small sequential comparisons and record every new signal before implementation. | Add more fixture rows only after the current one-command gate remains clean. | Any browser accessibility-tree signal repeatedly helps agent routing. |
...
| A24 | Local browser parity fixture gate | 100% | `pnpm compare:browser:fixture` compares one local browser-side semantic tree with `--agent-brief` across heading/link/button/field/image, table header/cell context, form state/relation context, list samples, and keyboard shortcuts. | Expand only with one fixture at a time and pre/post process checks. | Typecheck, extractor test, CLI test, browser fixture gate, static gates, README test, diff check, and process check pass. | +5% browser parity research. |
...
| Local browser parity fixture gate. | Browser-side semantic tree vs `--agent-brief` comparison. | P1 | Added `pnpm compare:browser:fixture`; the first local fixture covers heading/link/button/field/image, table header/cell context, form state/relation context, compacted list samples, and keyboard shortcut targeting. | Landed. |
...
| 22 | Later | Add the next local browser fixture only after the current single-fixture gate remains stable. | Pre/post `pnpm check:processes`; one browser-backed command per run. | Can raise or lower estimate. |
