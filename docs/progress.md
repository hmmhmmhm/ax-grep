...
| Browser-tree parity research | 88% | Six local browser fixtures now compare browser-side semantic output with `--agent-brief`, including range/value state; broader real-page and `agent-browser` comparison remains. |
...
- 88-96% reflects landed non-browser shortcuts, fallback-policy work, and the
  current six-fixture browser parity evidence.
...
| F4: Browser parity evidence set | Started, gated | A small sequential fixture set comparing static output against browser accessibility output. | Browser parity research moved to 88% after the sixth fixture proved range/value state parity; overall remains 96% because broader real-page and `agent-browser` comparison still remains. | Do not run broad browser checks; each run needs pre/post `pnpm check:processes`. |
...
| 4 | Classify browser-tree gaps. | 100% | The fixture set classified ARIA state normalization, compacted list samples, modal/live shallow shortcuts, selected-choice shortcuts, selected tab panel targets, selected gridcell shortcuts, and range/value state parity. | Every gap has priority, evidence, decision, validation command, and estimate impact. | Raised browser parity research to 88%. |
...
| Browser parity research | 88% | Six local browser fixture cases are in place and passed. | Expand the gap list as research finds new browser accessibility-tree signals; lower estimates if new important gaps appear. | Each gap is tagged `implement`, `browser-only`, or `defer` with priority and evidence. |
...
| P6: General accessibility parity | 88% | Run only small sequential comparisons and record every new signal before implementation. | Add more fixture rows only after the current one-command gate remains clean. | Any browser accessibility-tree signal repeatedly helps agent routing. |
...
| 37 | Later | Add the next local browser fixture only after the current six-fixture gate remains stable and no safer non-browser gap remains. | Pre/post `pnpm check:processes`; one browser-backed command per run. | Can raise or lower estimate. |
