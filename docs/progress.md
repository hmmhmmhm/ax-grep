...
| P2: Search result routing | 88% | Watch whether snippet provenance, official-source ambiguity, or failed-open categories need anything beyond top result/source shortcuts and decision reasons. | Keep current top choice shortcuts unless fixtures show agents need deeper dedupe/provenance. | Search/source fixtures show agents still need to inspect nested choices before opening or recovering. |
...
| 34 | Later | Add the next local browser fixture only after the current five-fixture gate remains stable and no non-browser gap remains. | Pre/post `pnpm check:processes`; one browser-backed command per run. | Can raise or lower estimate. |
