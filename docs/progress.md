...
| Page check handoff | 89% | Forms, action targets, table navigation shortcuts, hidden signal group counts/top shortcuts/selectors, author/source metadata, static-readiness reason codes/reasons, barrier-specific browser-capture reason codes, execution shortcuts, barriers, and read targets are exposed in full and brief modes. |
...
| 8 | Later | Compare one semantic table/list/control fixture against browser-tree output, sequentially only. | Pre/post `pnpm check:processes`; smallest comparison command recorded before use. | 1-3% semantic/browser parity. |
| 9 | Later | Recalculate the overall estimate after each landed shortcut or documented defer/browser-only decision. | Update this file in the same commit as the evidence. | Can raise or lower estimate. |
