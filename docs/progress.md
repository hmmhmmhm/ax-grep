...
| A4 | Browser fallback policy | 76% | Browser HTML reason/action/operation/args/capture shortcuts, static-readiness status/reason/readFrom shortcuts, and barrier-specific reason codes for challenge/login/paywall exist. | Add more observed low-content, client-rendered, and interaction-required categories. | Fixtures show clear `use static` or `need browser capture` reasons without manual interpretation. | +1-2% more to page handoff. |
