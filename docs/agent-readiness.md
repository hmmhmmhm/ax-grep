...
| A minimal `agent-browser` comparison has exact named-role overlap. | `pnpm readiness:agent-browser-smoke` runs `pnpm compare https://example.com` and requires an `agent-browser` snapshot, overlap `1.0`, and agent-readiness score `1.0`. | Covered as a single-target smoke gate; broader `agent-browser` comparison remains. |
