...
type CliSearchEngineAttemptShape = {
  engine?: string;
  ok?: boolean;
  resultCount?: number;
  error?: {
...
function summarizeCliEnvelope(envelope: unknown): CliAgentSummary {
  const item = envelope as {
    ...
    agent?: {
      ...
      topFormChoiceInvalidFieldInvalid?: boolean | string;
      ...
      semanticTopTableSampleCellRefs?: Array<{ path?: string; text?: string; rowIndex?: number; columnIndex?: number; rowSpan?: number; columnSpan?: number; headers?: string[]; rowHeaders?: string[]; columnHeaders?: string[]; selected?: boolean; current?: boolean | string; selector?: string; ownedTarget?: string }>;
      ...
      semanticTopTableFirstSampleCellCurrent?: boolean | string;
      ...
      semanticTopTableSecondSampleCellCurrent?: boolean | string;
      ...
      semanticTopTableFirstOwnedSampleCellCurrent?: boolean | string;
    ...
    searchEngines?: CliSearchEngineAttemptShape[];
  ...
  const summary: CliAgentSummary = {
    ...
    agentSourceSearchShortcutScore: scoreAgentSourceSearchShortcuts(item.agent, item.sourceSearch, item.error, item.searchEngines),
...
function scoreAgentTopChoiceShortcuts(agent: {
  ...
  topChoicePressed?: unknown;
  topChoiceExpanded?: boolean;
  topChoiceHaspopup?: unknown;
  ...
  if (expected.encodingType) {
    required += 1;
    if (agent.topChoiceEncodingType === expected.encodingType) matched += 1;
  } else if (agent.topChoiceEncodingType) {
    required += 1;
  }
  ...
  if (typeof expected.pressed !== "undefined") {
    required += 1;
    if (agent.topChoicePressed === expected.pressed) matched += 1;
  } else if (typeof agent.topChoicePressed !== "undefined") {
    required += 1;
  }
  if (typeof expected.expanded === "boolean") {
    required += 1;
    if (agent.topChoiceExpanded === expected.expanded) matched += 1;
  } else if (typeof agent.topChoiceExpanded === "boolean") {
    required += 1;
  }
  if (typeof expected.haspopup !== "undefined") {
    required += 1;
    if (agent.topChoiceHaspopup === expected.haspopup) matched += 1;
  } else if (typeof agent.topChoiceHaspopup !== "undefined") {
    required += 1;
  }
  if (expected.controls) {
    required += 1;
    if (agent.topChoiceControls === expected.controls) matched += 1;
  } else if (agent.topChoiceControls) {
    required += 1;
  }
  return roundScore(matched / required);
}
...
function scorePageCheckActionTargetUrlPaths(targets: unknown[]): number {
  const targetRecords = targets.filter((target): target is Record<string, unknown> => Boolean(target) && typeof target === "object");
  if (targetRecords.length === 0) return 1;
  let required = 0;
  let matched = 0;
  for (const target of targetRecords) {
    if (typeof target.targetUrl === "string") {
      required += 2;
      const targetUrlParts = compareUrlPathParts(target.targetUrl);
      if (optionalFieldMatches(target.targetUrlPath, targetUrlParts?.urlPath)) matched += 1;
      if (optionalFieldMatches(target.targetUrlQuery, targetUrlParts?.urlQuery)) matched += 1;
    }
    if (typeof target.urlTemplate === "string") {
      required += 2;
      const urlTemplateParts = compareUrlPathParts(target.urlTemplate);
      if (optionalFieldMatches(target.urlTemplatePath, urlTemplateParts?.urlPath)) matched += 1;
      if (optionalFieldMatches(target.urlTemplateQuery, urlTemplateParts?.urlQuery)) matched += 1;
    }
  }
  return required === 0 ? 1 : roundScore(matched / required);
}
...
function scoreAgentTopFormActionChoiceShortcuts(agent: {
  ...
  if (form) {
    ...
    if (agent?.topFormChoiceFirstFieldMin === firstField?.min) matched += 1;
    if (agent?.topFormChoiceFirstFieldMax === firstField?.max) matched += 1;
    if (agent?.topFormChoiceFirstFieldStep === firstField?.step) matched += 1;
    if (agent?.topFormChoiceFirstFieldMinLength === firstField?.minLength) matched += 1;
    if (agent?.topFormChoiceFirstFieldMaxLength === firstField?.maxLength) matched += 1;
    if (agent?.topFormChoiceFirstFieldRequired === firstField?.required) matched += 1;
    if (agent?.topFormChoiceFirstFieldChecked === firstField?.checked) matched += 1;
    if (agent?.topFormChoiceFirstFieldDisabled === firstField?.disabled) matched += 1;
    ...
    if (agent?.topFormChoiceInvalidFieldName === invalidField?.name) matched += 1;
    if (agent?.topFormChoiceInvalidFieldType === invalidField?.type) matched += 1;
    if (agent?.topFormChoiceInvalidFieldLabel === invalidField?.label) matched += 1;
    if (agent?.topFormChoiceInvalidFieldInvalid === invalidField?.invalid) matched += 1;
    if (agent?.topFormChoiceInvalidFieldSelector === invalidField?.selector) matched += 1;
  } else if (
    ...
    || agent?.topFormChoiceFirstFieldMin
    || agent?.topFormChoiceFirstFieldMax
    || agent?.topFormChoiceFirstFieldStep
    || typeof agent?.topFormChoiceFirstFieldMinLength === "number"
    || typeof agent?.topFormChoiceFirstFieldMaxLength === "number"
    || typeof agent?.topFormChoiceFirstFieldRequired === "boolean"
    || typeof agent?.topFormChoiceFirstFieldChecked === "boolean"
    || typeof agent?.topFormChoiceFirstFieldDisabled === "boolean"
    ...
    || agent?.topFormChoiceInvalidFieldName
    || agent?.topFormChoiceInvalidFieldType
    || agent?.topFormChoiceInvalidFieldLabel
    || typeof agent?.topFormChoiceInvalidFieldInvalid !== "undefined"
    || agent?.topFormChoiceInvalidFieldSelector
  ) {
    required += 1;
  } else {
    matched += 1;
...
function scoreAgentSourceSearchShortcuts(agent: {
  ...
  selectedResult?: CliAgentSourceSearchResultShape;
  alternateResults?: CliAgentSourceSearchResultShape[];
} | undefined, error?: unknown, searchEngines?: CliSearchEngineAttemptShape[]): number {
  if (!sourceSearch) {
    return typeof agent?.sourceSearchQuery === "undefined"
      ...
      && typeof agent?.sourceSearchEngineAttemptCount === "undefined"
      && typeof agent?.sourceSearchEngineSuccessCount === "undefined"
      && typeof agent?.sourceSearchEngineFailureCount === "undefined"
      && typeof agent?.sourceSearchFirstOkEngine === "undefined"
      && typeof agent?.sourceSearchFirstOkResultCount === "undefined"
      && typeof agent?.sourceSearchFirstFailedEngine === "undefined"
      && typeof agent?.sourceSearchFirstFailureCode === "undefined"
      && typeof agent?.sourceSearchFirstFailureStatus === "undefined"
  ...
  if (searchEngines?.length) {
    const okAttempts = searchEngines.filter((attempt) => attempt.ok);
    const failedAttempts = searchEngines.filter((attempt) => !attempt.ok);
    const firstOk = okAttempts[0];
    const firstFailed = failedAttempts[0];
    required += 3;
    if (agent?.sourceSearchEngineAttemptCount === searchEngines.length) matched += 1;
    if (agent?.sourceSearchEngineSuccessCount === okAttempts.length) matched += 1;
    if (agent?.sourceSearchEngineFailureCount === failedAttempts.length) matched += 1;
    if (firstOk) {
      if (firstOk.engine) {
        required += 1;
        if (agent?.sourceSearchFirstOkEngine === firstOk.engine) matched += 1;
      }
      if (typeof firstOk.resultCount === "number") {
        required += 1;
        if (agent?.sourceSearchFirstOkResultCount === firstOk.resultCount) matched += 1;
      }
    }
    if (firstFailed) {
      if (firstFailed.engine) {
        required += 1;
        if (agent?.sourceSearchFirstFailedEngine === firstFailed.engine) matched += 1;
      }
      if (firstFailed.error?.code) {
        required += 1;
        if (agent?.sourceSearchFirstFailureCode === firstFailed.error.code) matched += 1;
      }
      if (typeof firstFailed.error?.status === "number") {
        required += 1;
        if (agent?.sourceSearchFirstFailureStatus === firstFailed.error.status) matched += 1;
      } else if (typeof agent?.sourceSearchFirstFailureStatus === "number") {
        required += 1;
      }
    }
  }
  if (selected) {
    ...
    if (selected.snippet) {
      required += 1;
      if (agent?.sourceSearchSelectedSnippet === selected.snippet) matched += 1;
    } else if (agent?.sourceSearchSelectedSnippet) {
      required += 1;
    }
    if (selected.dateText) {
      required += 1;
      if (agent?.sourceSearchSelectedDateText === selected.dateText) matched += 1;
    ...
    } else if (agent?.sourceSearchSelectedMatchedTerm) {
      required += 1;
    }
    if (selected.findMatches?.[0]) {
      required += 1;
      if (agent?.sourceSearchSelectedFindMatch === selected.findMatches[0]) matched += 1;
    } else if (agent?.sourceSearchSelectedFindMatch) {
      required += 1;
    }
    if (selected.sitelinks?.length) {
      required += 6;
  ...
  if (alternate) {
    ...
    } else if (agent?.sourceSearchAlternateSnippet) {
      required += 1;
    }
    if (alternate.dateText) {
      required += 1;
      if (agent?.sourceSearchAlternateDateText === alternate.dateText) matched += 1;
    ...
    } else if (agent?.sourceSearchAlternateMatchedTerm) {
      required += 1;
    }
    if (alternate.findMatches?.[0]) {
      required += 1;
      if (agent?.sourceSearchAlternateFindMatch === alternate.findMatches[0]) matched += 1;
    } else if (agent?.sourceSearchAlternateFindMatch) {
      required += 1;
    }
    if (alternate.sitelinks?.length) {
      required += 6;
  ...
  if (sourceSearch.alternateResults?.length) {
    ...
    for (const [index, choice] of comparableChoices.entries()) {
      ...
      if (choice.snippet) {
        required += 1;
        if (agentChoice?.snippet === choice.snippet) matched += 1;
      } else if (agentChoice?.snippet) {
        required += 1;
      }
      if (choice.dateText) {
        required += 1;
        if (agentChoice?.dateText === choice.dateText) matched += 1;
      } else if (agentChoice?.dateText) {
        required += 1;
      }
      if (choice.matchedTerms?.[0]) {
        required += 1;
        if (agentChoice?.matchedTerms?.[0] === choice.matchedTerms[0]) matched += 1;
      } else if (agentChoice?.matchedTerms?.length) {
        required += 1;
      }
      if (choice.findMatches?.[0]) {
        required += 1;
        if (agentChoice?.findMatches?.[0] === choice.findMatches[0]) matched += 1;
      } else if (agentChoice?.findMatches?.length) {
        required += 1;
      }
...
function scoreOwnedSampleCellShortcuts(agentRecord: Record<string, unknown>, sampleCellRefs: unknown): { matched: number; required: number } {
  if (!Array.isArray(sampleCellRefs)) return { matched: 0, required: 0 };
  const firstOwnedSampleCellRef = sampleCellRefs.find((cell): cell is { path?: unknown; text?: unknown; rowIndex?: unknown; columnIndex?: unknown; rowSpan?: unknown; columnSpan?: unknown; headers?: unknown; rowHeaders?: unknown; columnHeaders?: unknown; selected?: unknown; current?: unknown; selector?: unknown; ownedTarget?: unknown } => {
    const ownedTarget = cell && typeof cell === "object" ? (cell as { ownedTarget?: unknown }).ownedTarget : undefined;
    return typeof ownedTarget === "string" && ownedTarget.length > 0;
  });
  if (!firstOwnedSampleCellRef) return { matched: 0, required: 0 };
  let matched = 0;
  const required = 14;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellPath === firstOwnedSampleCellRef.path) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellText === firstOwnedSampleCellRef.text) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellRowIndex === firstOwnedSampleCellRef.rowIndex) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellColumnIndex === firstOwnedSampleCellRef.columnIndex) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellRowSpan === firstOwnedSampleCellRef.rowSpan) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellColumnSpan === firstOwnedSampleCellRef.columnSpan) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableFirstOwnedSampleCellHeaders) === JSON.stringify(firstOwnedSampleCellRef.headers)) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableFirstOwnedSampleCellRowHeaders) === JSON.stringify(firstOwnedSampleCellRef.rowHeaders)) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableFirstOwnedSampleCellColumnHeaders) === JSON.stringify(firstOwnedSampleCellRef.columnHeaders)) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellSelected === firstOwnedSampleCellRef.selected) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellCurrent === firstOwnedSampleCellRef.current) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellSelector === firstOwnedSampleCellRef.selector) matched += 1;
  if (agentRecord.semanticTopTableFirstOwnedSampleCellOwnedTarget === firstOwnedSampleCellRef.ownedTarget) matched += 1;
  if (typeof agentRecord.semanticTopTableFirstOwnedSampleCellText === "string" && agentRecord.semanticTopTableFirstOwnedSampleCellText.length > 0) matched += 1;
  return { matched, required };
}

function scoreSecondSampleCellShortcuts(agentRecord: Record<string, unknown>, sampleCellRefs: unknown): { matched: number; required: number } {
  const secondSampleCellRef = Array.isArray(sampleCellRefs)
    ? sampleCellRefs[1] as { path?: unknown; text?: unknown; rowIndex?: unknown; columnIndex?: unknown; rowSpan?: unknown; columnSpan?: unknown; headers?: unknown; rowHeaders?: unknown; columnHeaders?: unknown; selected?: unknown; current?: unknown; selector?: unknown; ownedTarget?: unknown } | undefined
    : undefined;
  if (!secondSampleCellRef) return { matched: 0, required: 0 };
  let matched = 0;
  const required = 14;
  if (agentRecord.semanticTopTableSecondSampleCellPath === secondSampleCellRef.path) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellText === secondSampleCellRef.text) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellRowIndex === secondSampleCellRef.rowIndex) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellColumnIndex === secondSampleCellRef.columnIndex) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellRowSpan === secondSampleCellRef.rowSpan) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellColumnSpan === secondSampleCellRef.columnSpan) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableSecondSampleCellHeaders) === JSON.stringify(secondSampleCellRef.headers)) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableSecondSampleCellRowHeaders) === JSON.stringify(secondSampleCellRef.rowHeaders)) matched += 1;
  if (JSON.stringify(agentRecord.semanticTopTableSecondSampleCellColumnHeaders) === JSON.stringify(secondSampleCellRef.columnHeaders)) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellSelected === secondSampleCellRef.selected) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellCurrent === secondSampleCellRef.current) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellSelector === secondSampleCellRef.selector) matched += 1;
  if (agentRecord.semanticTopTableSecondSampleCellOwnedTarget === secondSampleCellRef.ownedTarget) matched += 1;
  if (typeof agentRecord.semanticTopTableSecondSampleCellText === "string" && agentRecord.semanticTopTableSecondSampleCellText.length > 0) matched += 1;
  return { matched, required };
}
...
function scoreCurrentLinkShortcuts(agentRecord: Record<string, unknown>, links: unknown): { matched: number; required: number } {
  ...
  if (currentLink) {
    let matched = 0;
    const required = 12;
    const currentLinkUrlParts = typeof currentLink.url === "string" ? compareUrlPathParts(currentLink.url) : undefined;
    ...
    if (currentLinkUrlParts?.urlPath) {
      matched += agentRecord.semanticTopCurrentLinkUrlPath === currentLinkUrlParts.urlPath ? 1 : 0;
    } else if (agentRecord.semanticTopCurrentLinkUrlPath) {
      return { matched, required: required + 1 };
    }
    if (currentLinkUrlParts?.urlQuery) {
      matched += agentRecord.semanticTopCurrentLinkUrlQuery === currentLinkUrlParts.urlQuery ? 1 : 0;
    } else if (agentRecord.semanticTopCurrentLinkUrlQuery) {
      return { matched, required: required + 1 };
    }
    ...
    return { matched, required: required + (currentLinkUrlParts?.urlPath ? 1 : 0) + (currentLinkUrlParts?.urlQuery ? 1 : 0) };
  }
  if (
    agentRecord.semanticTopCurrentLinkName
    || agentRecord.semanticTopCurrentLinkPath
    || agentRecord.semanticTopCurrentLinkUrl
    || agentRecord.semanticTopCurrentLinkUrlPath
    || agentRecord.semanticTopCurrentLinkUrlQuery
...
function scoreAgentSemanticSummary(agent: Record<string, any> | undefined): number {
  ...
  if (link && typeof link.url === "string") {
    ...
    const linkUrlParts = compareUrlPathParts(link.url);
    if (linkUrlParts?.urlPath) {
      required += 1;
      if (agent?.semanticTopLinkUrlPath === linkUrlParts.urlPath) matched += 1;
    } else if (agent?.semanticTopLinkUrlPath) {
      required += 1;
    }
    if (linkUrlParts?.urlQuery) {
      required += 1;
      if (agent?.semanticTopLinkUrlQuery === linkUrlParts.urlQuery) matched += 1;
    } else if (agent?.semanticTopLinkUrlQuery) {
      required += 1;
    }
  }
  ...
  const firstSampleCellRef = Array.isArray(table?.sampleCellRefs) ? table.sampleCellRefs[0] as { path?: unknown; text?: unknown; rowIndex?: unknown; columnIndex?: unknown; rowSpan?: unknown; columnSpan?: unknown; headers?: unknown; rowHeaders?: unknown; columnHeaders?: unknown; selected?: unknown; current?: unknown; selector?: unknown; ownedTarget?: unknown } | undefined : undefined;
  if (firstSampleCellRef) {
    ...
    if (agentRecord.semanticTopTableFirstSampleCellSelected === firstSampleCellRef.selected) matched += 1;
    if (agentRecord.semanticTopTableFirstSampleCellCurrent === firstSampleCellRef.current) matched += 1;
    if (agent?.semanticTopTableFirstSampleCellSelector === firstSampleCellRef.selector) matched += 1;
    if (agentRecord.semanticTopTableFirstSampleCellOwnedTarget === firstSampleCellRef.ownedTarget) matched += 1;
    if (typeof agent?.semanticTopTableFirstSampleCellText === "string" && agent.semanticTopTableFirstSampleCellText.length > 0) matched += 1;
  }
  const secondSampleScore = scoreSecondSampleCellShortcuts(agentRecord, table?.sampleCellRefs);
  matched += secondSampleScore.matched;
  required += secondSampleScore.required;
...
function scoreAgentStructuredShortcuts(agent: {
  ...
  if (topEmbed) {
    required += 5;
    if (agent.topEmbedPath === topEmbed.path) matched += 1;
    if (agent.topEmbedKind === topEmbed.kind) matched += 1;
    if (agent.topEmbedUrl === topEmbed.url) matched += 1;
    if (topEmbed.url) {
      const topEmbedUrlParts = compareUrlPathParts(topEmbed.url);
      if (topEmbedUrlParts?.urlPath) {
        required += 1;
        if (agent.topEmbedUrlPath === topEmbedUrlParts.urlPath) matched += 1;
      } else if (agent.topEmbedUrlPath) {
        required += 1;
      }
      if (topEmbedUrlParts?.urlQuery) {
        required += 1;
        if (agent.topEmbedUrlQuery === topEmbedUrlParts.urlQuery) matched += 1;
      } else if (agent.topEmbedUrlQuery) {
        required += 1;
      }
    }
    if (agent.topEmbedTitle === topEmbed.title) matched += 1;
    if (agent.topEmbedSelector === topEmbed.selector) matched += 1;
  ...
  } else if (agent.topEmbedPath || agent.topEmbedKind || agent.topEmbedUrl || agent.topEmbedUrlPath || agent.topEmbedUrlQuery || agent.topEmbedTitle || agent.topEmbedSelector || agent.topEmbedCommand || agent.topEmbedCommandArgs) {
    required += 1;
  }
  ...
  if (topTranscript) {
    required += 6;
    if (agent.topTranscriptPath === topTranscript.path) matched += 1;
    if (agent.topTranscriptKind === topTranscript.kind) matched += 1;
    if (agent.topTranscriptUrl === topTranscript.url) matched += 1;
    if (topTranscript.url) {
      const topTranscriptUrlParts = compareUrlPathParts(topTranscript.url);
      if (topTranscriptUrlParts?.urlPath) {
        required += 1;
        if (agent.topTranscriptUrlPath === topTranscriptUrlParts.urlPath) matched += 1;
      } else if (agent.topTranscriptUrlPath) {
        required += 1;
      }
      if (topTranscriptUrlParts?.urlQuery) {
        required += 1;
        if (agent.topTranscriptUrlQuery === topTranscriptUrlParts.urlQuery) matched += 1;
      } else if (agent.topTranscriptUrlQuery) {
        required += 1;
      }
    }
    if (agent.topTranscriptLabel === topTranscript.label) matched += 1;
    if (agent.topTranscriptLanguage === topTranscript.language) matched += 1;
    if (agent.topTranscriptSelector === topTranscript.selector) matched += 1;
  ...
  } else if (agent.topTranscriptPath || agent.topTranscriptKind || agent.topTranscriptUrl || agent.topTranscriptUrlPath || agent.topTranscriptUrlQuery || agent.topTranscriptLabel || agent.topTranscriptLanguage || agent.topTranscriptSelector || agent.topTranscriptCommand || agent.topTranscriptCommandArgs) {
    required += 1;
  }
...
function scoreAgentAlternativeActionShortcuts(agent: {
  ...
  if (!action) {
    return agent?.alternativeActionName
      ...
      || agent?.alternativeActionTargetHost
      ...
      || agent?.alternativeActionTargetText
  ...
  if (action.target?.url) {
    required += 1;
    if (agent?.alternativeActionTargetUrl === action.target.url) matched += 1;
  } else if (agent?.alternativeActionTargetUrl) {
    required += 1;
  }
  if (action.target?.path) {
    required += 1;
    if (agent?.alternativeActionTargetPath === action.target.path) matched += 1;
  } else if (agent?.alternativeActionTargetPath) {
    required += 1;
  }
  if (action.target?.title) {
    required += 1;
    if (agent?.alternativeActionTargetTitle === action.target.title) matched += 1;
  } else if (agent?.alternativeActionTargetTitle) {
    required += 1;
  }
  if (action.target?.host) {
    required += 1;
    if (agent?.alternativeActionTargetHost === action.target.host) matched += 1;
  } else if (agent?.alternativeActionTargetHost) {
    required += 1;
  }
  if (action.target?.source) {
    required += 1;
    if (agent?.alternativeActionTargetSource === action.target.source) matched += 1;
  } else if (agent?.alternativeActionTargetSource) {
    required += 1;
  }
  if (typeof action.target?.rank === "number") {
    required += 1;
    if (agent?.alternativeActionTargetRank === action.target.rank) matched += 1;
  } else if (typeof agent?.alternativeActionTargetRank === "number") {
    required += 1;
  }
  if (typeof action.target?.sourceScore === "number") {
    required += 1;
    if (agent?.alternativeActionTargetSourceScore === action.target.sourceScore) matched += 1;
  ...
  } else if (agent?.alternativeActionTargetRelevance) {
    required += 1;
  }
  if (typeof action.target?.isLikelyOfficial === "boolean") {
    required += 1;
    if (agent?.alternativeActionTargetLikelyOfficial === action.target.isLikelyOfficial) matched += 1;
  } else if (typeof agent?.alternativeActionTargetLikelyOfficial === "boolean") {
    required += 1;
  }
  if (action.target?.selector) {
    required += 1;
    if (agent?.alternativeActionTargetSelector === action.target.selector) matched += 1;
  } else if (agent?.alternativeActionTargetSelector) {
    required += 1;
  }
  if (action.target?.text) {
    required += 1;
    if (agent?.alternativeActionTargetText === action.target.text) matched += 1;
  } else if (agent?.alternativeActionTargetText) {
    required += 1;
  }
  if (action.requiresBrowserInteraction) {
    required += 1;
...
function scoreAgentHandoffShortcuts(agent: {
  ...
  if (handoff.afterInteractionCommand) {
    required += 1;
    if (agent.handoffAfterInteractionCommand === handoff.afterInteractionCommand) matched += 1;
  } else if (agent.handoffAfterInteractionCommand) {
    required += 1;
  }
  if (handoff.afterInteractionCommandArgs) {
    required += 1;
    if (JSON.stringify(agent.handoffAfterInteractionCommandArgs) === JSON.stringify(handoff.afterInteractionCommandArgs)) matched += 1;
  } else if (agent.handoffAfterInteractionCommandArgs) {
    required += 1;
  }
  if (handoff.readFrom) {
    required += 1;
...
function scoreCliAgentSummary(summary: CliAgentSummary): number {
  ...
  const score = roundScore(Math.min(1,
    ...
    + summary.pageCheckActionTargetUrlPathScore * 0.005
